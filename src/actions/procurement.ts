...
export async function autoGenerateDraftPOs(prId: string, userId: string): Promise<void> {
  try {
    ...
    // Group items by preferred supplier (skip items without a supplier — those need manual PO creation)
    const groups = new Map<string, typeof items>();
    for (const item of items) {
      if (!item.preferredSupplierId) continue;
      if (!groups.has(item.preferredSupplierId)) groups.set(item.preferredSupplierId, []);
      groups.get(item.preferredSupplierId)!.push(item);
    }
