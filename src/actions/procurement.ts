...
export async function approvePr(id: string): Promise<SimpleResult> {
  ...
  // Auto-generate draft POs grouped by preferred supplier
  void autoGenerateDraftPOs(id, user.id);
...
async function triggerBatchingIPOsForPR(prId: string, userId: string): Promise<void> {
  try {
    ...
    for (const item of items) {
      ...
      // BOM explosion + raw material PR generation happens when Batching accepts the IPO
      // (see updateIPOStatus in batching-bom.ts)
    }
  } catch {
...
// ── Auto-generate draft POs from an approved PR, grouped by preferred supplier ──
// Items without a preferred supplier get a single "unassigned" PO (no supplier).
export async function autoGenerateDraftPOs(prId: string, userId: string): Promise<void> {
  try {
    const [pr] = await db
      .select({ projectId: purchaseRequisitions.projectId })
      .from(purchaseRequisitions)
      .where(eq(purchaseRequisitions.id, prId))
      .limit(1);
    if (!pr) return;
    ...
    // Exclude premix materials (fulfilled via IPO, not vendor PO)
    const premixIds = await getPremixMaterialIds();
    const items = allItems.filter((item) => !item.materialId || !premixIds.has(item.materialId));
    if (items.length === 0) return;

    // Check if POs already exist for this PR
    const [existingPo] = await db
      .select({ id: purchaseOrders.id })
      .from(purchaseOrders)
      .where(eq(purchaseOrders.prId, prId))
      .limit(1);
    if (existingPo) return;

    // Group items by preferred supplier; items without a supplier go into one "unassigned" PO
    const NO_SUPPLIER = "__no_supplier__";
    const groups = new Map<string, typeof items>();
    for (const item of items) {
      const key = item.preferredSupplierId ?? NO_SUPPLIER;
    ...
    // Non-blocking — PO generation failure does not roll back PR approval
  }
}
...
// ─── Admin: Delete PR ────────────────────────────────────────────────────────
export async function adminDeletePr(id: string): Promise<SimpleResult> {
  const user = await getAuthUser();
  if (!user) return { success: false, error: "Not authenticated." };
  const { isAdminOrBod } = await import("@/lib/supabase-server");
  if (!(await isAdminOrBod())) return { success: false, error: "Admin/BOD only." };

  // Delete linked PO items, POs, then PR items, then the PR itself
  ...
  await db.delete(purchaseOrders).where(eq(purchaseOrders.prId, id));
  await db.delete(purchaseRequisitionItems).where(eq(purchaseRequisitionItems.prId, id));
...
// ─── Admin: Delete PO ────────────────────────────────────────────────────────
export async function adminDeletePo(id: string): Promise<SimpleResult> {
  const user = await getAuthUser();
  if (!user) return { success: false, error: "Not authenticated." };
  const { isAdminOrBod } = await import("@/lib/supabase-server");
  if (!(await isAdminOrBod())) return { success: false, error: "Admin/BOD only." };
