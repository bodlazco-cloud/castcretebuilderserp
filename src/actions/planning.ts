...
import { autoGenerateDraftPOs } from "./procurement";
...
export async function raiseMrpPurchaseRequisition(
  ...
  // MRP forecasts already passed two-tier approval (Manager + BOD),
  // so the PR is created as APPROVED and goes straight to Procurement for PO creation
  const [pr] = await db
  ...
  await db.insert(purchaseRequisitionItems).values({
    ...
    preferredSupplierId: forecast.preferredSupplierId ?? null,
  });
  ...
  // Auto-generate draft PO(s) for items with a preferred supplier
  void autoGenerateDraftPOs(pr.id, user.id);
