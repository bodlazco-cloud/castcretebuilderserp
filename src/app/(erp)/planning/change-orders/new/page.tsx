...
export default async function NewChangeOrderPage() {
  ...
  let activityRows: { id: string; projectId: string; activityCode: string; activityName: string; scopeName: string }[] = [];
  let materialRows: { id: string; code: string; name: string; unit: string }[] = [];
  let dbError: string | null = null;

  try {
    [projectRows, activityRows, materialRows] = await Promise.all([
      ...
      <div style={{ maxWidth: "760px" }}>
        ...
        {dbError && (
          <div style={{ padding: "1rem 1.25rem", marginBottom: "1.5rem", borderRadius: "8px", background: "#fef2f2", border: "1px solid #fecaca", color: "#991b1b", fontSize: "0.875rem" }}>
            ...
            <br /><span style={{ fontSize: "0.8rem" }}>Run migration <code>004_admin_master_lists.sql</code> in Supabase SQL Editor to create the required tables.</span>
          </div>
        )}
