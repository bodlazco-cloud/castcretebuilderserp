...
const categoryCodeMap: Record<string, string> = {
  ...
  SPECIALTY_WORKS:  "SPECIALTY_WORKS",
  MEPF:             "MEPF",
...
export function IssueNtpForm({
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const categoryValue  = categoryCodeMap[selectedCatObj?.code ?? ""] as any;
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      {/* Construction Phase: Category + Scope of Work */}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
          <label>
            ...
            <select
              ...
              onChange={(e) => { setSelectedCategory(e.target.value); setSelectedScope(""); }}
              style={inputStyle}
            >
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: "0.75rem", marginBottom: "0.65rem", flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: "0.78rem", color: "#6b7280", display: "flex", alignItems: "center" }}>
              {visibleUnits.length} shown · {checkedUnits.size} selected
            </span>
          </div>
              ...
              <div style={{ maxHeight: "260px", overflowY: "auto" }}>
                {visibleUnits.map((u) => (
                  <label key={u.id} style={{
                    ...
                    <input type="checkbox" checked={checkedUnits.has(u.id)}
                      onChange={() => toggleUnit(u.id)} style={{ cursor: "pointer" }} />
