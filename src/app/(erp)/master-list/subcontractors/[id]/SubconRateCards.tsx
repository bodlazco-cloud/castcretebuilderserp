...
function AddDeductionForm({ rateCardId, onAdded }: { rateCardId: string; onAdded: () => void }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await createSubconRateCardDeduction(rateCardId, { name, deductionPct: Number(pct) / 100 });
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", gap: "0.5rem", alignItems: "flex-end", marginTop: "0.5rem" }}>
      <div style={{ flex: 2 }}>
        ...
        <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Admin fee" style={{ ...inputStyle, padding: "0.4rem 0.6rem" }} />
      </div>
      <div style={{ flex: 1 }}>
...
export function SubconRateCards({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setUnitModel(""); setUnitType(""); setRatePerUnit(""); setRetentionPct("10");
        router.refresh();
      } else {
  ...
  function handleDeleteDeduction(id: string) {
    startTransition(async () => {
      await deleteSubconRateCardDeduction(id);
      router.refresh();
    });
  }
