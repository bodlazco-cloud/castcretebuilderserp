...
-- Helper: extract dept_code from the JWT (must live in public schema — auth schema is restricted)
CREATE OR REPLACE FUNCTION public.dept_code()
...
CREATE POLICY "admin_write_materials"   ON materials            FOR ALL USING (public.dept_code() IN ('ADMIN','BOD'));
CREATE POLICY "admin_write_bom"         ON bom_standards        FOR ALL USING (public.dept_code() IN ('ADMIN','BOD','PLANNING'));
CREATE POLICY "admin_write_activities"  ON activity_definitions FOR ALL USING (public.dept_code() IN ('ADMIN','BOD','PLANNING'));
CREATE POLICY "admin_write_milestones"  ON milestone_definitions FOR ALL USING (public.dept_code() IN ('ADMIN','BOD','PLANNING'));
CREATE POLICY "admin_write_settings"    ON admin_settings       FOR ALL USING (public.dept_code() IN ('ADMIN','BOD'));
CREATE POLICY "admin_write_rate_cards"  ON developer_rate_cards FOR ALL USING (public.dept_code() IN ('ADMIN','BOD'));

-- ─── PLANNING ─────────────────────────────────────────────────────────────────
CREATE POLICY "planning_write_pr"       ON purchase_requisitions      FOR ALL USING (public.dept_code() IN ('PLANNING','ADMIN','BOD'));
CREATE POLICY "planning_write_pr_items" ON purchase_requisition_items FOR ALL USING (public.dept_code() IN ('PLANNING','ADMIN','BOD'));
CREATE POLICY "planning_read_subcon"    ON subcontractor_capacity_matrix FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "planning_write_subcon_cap" ON subcontractor_capacity_matrix FOR ALL USING (public.dept_code() IN ('PLANNING','ADMIN','BOD'));
CREATE POLICY "planning_write_mix"      ON mix_designs                FOR ALL USING (public.dept_code() IN ('PLANNING','ADMIN','BOD'));
CREATE POLICY "planning_read_unit_act"  ON unit_activities            FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "planning_write_eot"      ON eot_requests               FOR ALL USING (public.dept_code() IN ('PLANNING','ADMIN','BOD'));

-- ─── CONSTRUCTION / OPERATIONS ───────────────────────────────────────────────
CREATE POLICY "ops_write_task_assign"   ON task_assignments       FOR ALL USING (public.dept_code() IN ('CONSTRUCTION','ADMIN','BOD'));
CREATE POLICY "ops_write_daily_prog"    ON daily_progress_entries FOR ALL USING (public.dept_code() IN ('CONSTRUCTION','ADMIN','BOD'));
CREATE POLICY "ops_write_war"           ON work_accomplished_reports FOR ALL
    USING (public.dept_code() IN ('CONSTRUCTION','FINANCE','AUDIT','ADMIN','BOD'));
CREATE POLICY "ops_write_unit_milestones" ON unit_milestones      FOR ALL USING (public.dept_code() IN ('CONSTRUCTION','AUDIT','ADMIN','BOD'));
...
CREATE POLICY "proc_write_po"           ON purchase_orders        FOR ALL USING (public.dept_code() IN ('PROCUREMENT','ADMIN','BOD'));
...
CREATE POLICY "proc_write_mrr"          ON material_receiving_reports FOR ALL USING (public.dept_code() IN ('PROCUREMENT','ADMIN','BOD'));
CREATE POLICY "proc_write_mrr_items"    ON mrr_items              FOR ALL USING (public.dept_code() IN ('PROCUREMENT','ADMIN','BOD'));
CREATE POLICY "proc_write_transfers"    ON material_transfers     FOR ALL USING (public.dept_code() IN ('PROCUREMENT','ADMIN','BOD'));
...
CREATE POLICY "proc_write_stock"        ON inventory_stock        FOR ALL USING (public.dept_code() IN ('PROCUREMENT','ADMIN','BOD'));
...
CREATE POLICY "audit_write_docs"        ON milestone_documents    FOR ALL USING (public.dept_code() IN ('AUDIT','CONSTRUCTION','PROCUREMENT','FINANCE','ADMIN','BOD'));
CREATE POLICY "audit_read_docs"         ON milestone_documents    FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "audit_write_subcon_perf" ON subcontractor_performance_ratings FOR ALL USING (public.dept_code() IN ('AUDIT','PLANNING','ADMIN','BOD'));

-- ─── BATCHING ─────────────────────────────────────────────────────────────────
CREATE POLICY "batch_write_logs"        ON batching_production_logs  FOR ALL USING (public.dept_code() IN ('BATCHING','ADMIN','BOD'));
CREATE POLICY "batch_write_notes"       ON concrete_delivery_notes   FOR ALL USING (public.dept_code() IN ('BATCHING','ADMIN','BOD'));
CREATE POLICY "batch_write_receipts"    ON concrete_delivery_receipts FOR ALL USING (public.dept_code() IN ('BATCHING','CONSTRUCTION','ADMIN','BOD'));
CREATE POLICY "batch_write_sales"       ON batching_internal_sales   FOR ALL USING (public.dept_code() IN ('BATCHING','ADMIN','BOD'));
CREATE POLICY "batch_read_all"          ON batching_production_logs  FOR SELECT USING (auth.role() = 'authenticated');

-- ─── MOTORPOOL ────────────────────────────────────────────────────────────────
CREATE POLICY "fleet_write_equip"       ON equipment              FOR ALL USING (public.dept_code() IN ('MOTORPOOL','ADMIN','BOD'));
CREATE POLICY "fleet_write_assignments" ON equipment_assignments  FOR ALL USING (public.dept_code() IN ('MOTORPOOL','ADMIN','BOD'));
CREATE POLICY "fleet_write_maintenance" ON maintenance_records    FOR ALL USING (public.dept_code() IN ('MOTORPOOL','ADMIN','BOD'));
CREATE POLICY "fleet_write_fuel"        ON fuel_logs              FOR ALL USING (public.dept_code() IN ('MOTORPOOL','ADMIN','BOD'));
CREATE POLICY "fleet_write_checklist"   ON equipment_daily_checklists FOR ALL USING (public.dept_code() IN ('MOTORPOOL','ADMIN','BOD'));
CREATE POLICY "fleet_write_flip"        ON fix_or_flip_assessments FOR ALL USING (public.dept_code() IN ('MOTORPOOL','ADMIN','BOD'));
...
CREATE POLICY "finance_write_invoices"  ON invoices               FOR ALL USING (public.dept_code() IN ('FINANCE','AUDIT','ADMIN','BOD'));
CREATE POLICY "finance_write_payables"  ON payables               FOR ALL USING (public.dept_code() IN ('FINANCE','AUDIT','ADMIN','BOD'));
CREATE POLICY "finance_write_vouchers"  ON manual_vouchers        FOR ALL USING (public.dept_code() IN ('FINANCE','ADMIN','BOD'));
CREATE POLICY "finance_write_payments"  ON payment_requests       FOR ALL USING (public.dept_code() IN ('FINANCE','ADMIN','BOD'));
CREATE POLICY "finance_write_ledger"    ON financial_ledger       FOR ALL USING (public.dept_code() IN ('FINANCE','ADMIN','BOD'));
CREATE POLICY "finance_write_cashflow"  ON cash_flow_projections  FOR ALL USING (public.dept_code() IN ('FINANCE','ADMIN','BOD'));
CREATE POLICY "finance_write_advance"   ON developer_advance_tracker FOR ALL USING (public.dept_code() IN ('FINANCE','ADMIN','BOD'));
CREATE POLICY "finance_read_invoices"   ON invoices               FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "finance_read_ledger"     ON financial_ledger       FOR SELECT USING (public.dept_code() IN ('FINANCE','AUDIT','ADMIN','BOD'));
...
CREATE POLICY "finance_write_osm"       ON osm_deduction_buckets  FOR ALL USING (public.dept_code() IN ('FINANCE','PROCUREMENT','ADMIN','BOD'));
...
CREATE POLICY "hr_write_employees"      ON employees              FOR ALL USING (public.dept_code() IN ('HR','ADMIN','BOD'));
CREATE POLICY "hr_read_employees"       ON employees              FOR SELECT USING (public.dept_code() IN ('HR','ADMIN','BOD'));
CREATE POLICY "hr_write_dtr"            ON daily_time_records     FOR ALL USING (public.dept_code() IN ('HR','ADMIN','BOD'));
CREATE POLICY "hr_read_dtr"             ON daily_time_records     FOR SELECT USING (public.dept_code() IN ('HR','ADMIN','BOD'));
CREATE POLICY "hr_write_payroll"        ON payroll_records        FOR ALL USING (public.dept_code() IN ('HR','ADMIN','BOD'));
CREATE POLICY "hr_read_payroll"         ON payroll_records        FOR SELECT USING (public.dept_code() IN ('HR','ADMIN','BOD'));
CREATE POLICY "hr_write_leave"          ON leave_schedules        FOR ALL USING (public.dept_code() IN ('HR','ADMIN','BOD'));
CREATE POLICY "hr_read_leave"           ON leave_schedules        FOR SELECT USING (public.dept_code() IN ('HR','ADMIN','BOD'));
-- Construction can see leave schedules to anticipate manpower drops
CREATE POLICY "ops_read_leave"          ON leave_schedules        FOR SELECT USING (public.dept_code() IN ('CONSTRUCTION','PLANNING','ADMIN','BOD'));
